...
then
   cd $THIS_TOP
   git  clone  git@172.16.8.9:sysdev/packages/linux_lsp/boot/amboot   -b  sysdev_kdm_v1.2
   cd -
else
   cd $AMBOOT_DIR
   git  pull  --rebase
   git  checkout   -b   sysdev_kdm_v1.2  origin/sysdev_kdm_v1.2
   git  checkout   sysdev_kdm_v1.2
   cd -
fi
