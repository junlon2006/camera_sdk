...
function InitSchedule()
{
	setTimeout(function()
	{
		...
		var weekcn=new Array(getNodeValue('MMonday'),getNodeValue('MTuesday'),getNodeValue('MWednesday'),getNodeValue('MThursday'),getNodeValue('MFriday'),getNodeValue('MSaturday'),getNodeValue('MSunday'));
...
function CheckTimeSetValid()
{
	...
	for(i = 1;i <= m_iTimePart; i++)
	{
		...
		{
			szRetInfo = szAreaNameInfo + getNodeValue('MTimeslot') + i + getNodeValue('jsTimeSegmentErrorSegTips');
			$("#SetResultTipsTwo").html(szRetInfo); 
			return false;
		}
		else if(stoptime > 2400)                      //结束时间不能大于24:00
		{
			szRetInfo = szAreaNameInfo + getNodeValue('MTimeslot') + i + getNodeValue('EndTimeRangeTips');
			$("#SetResultTipsTwo").html(szRetInfo); 
			return false;
		}
		else
	...
	for(i = 1;i <= m_iTimePart;i++)
	{
		...
		for(j = 1;j <= m_iTimePart;j++)
		{
			if(i != j)
			{
				...
				{
					szRetInfo = szAreaNameInfo + getNodeValue('MTimeslot')+ i + getNodeValue('WithTimeSegmentTips') + j + getNodeValue('OverlappedTips');
					$("#SetResultTipsTwo").html(szRetInfo); 
					return false;
				}	
			}
